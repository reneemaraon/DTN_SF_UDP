...
class StationaryDtnApp : public Application{
public:
  
  StationaryDtnApp ();
  virtual ~StationaryDtnApp();
  
  void Setup (Ptr<Node> node); //ok
  void DstHandleConnectionCreated (Ptr<Socket> s, const Address & addr); //ok
  void ReceiveHello (Ptr<Socket> socket); //ok 
  void ScheduleTx (uint32_t dstnode, Time tNext, uint32_t packetSize); //ok
  void SendHello (Ptr<Socket> socket, double endTime, Time pktInterval, uint32_t first); //okay na wala 'to
  void Retransmit (InetSocketAddress sendTo, int32_t id, int32_t retx);//ok
  void SendMore (InetSocketAddress sendTo, int32_t id, int32_t retx); //ok
  void ConnectionSucceeds (Ptr<Socket> localSocket); //ok
  void ConnectionFails (Ptr<Socket> localSocket); //ok
  void ReceiveBundle (Ptr<Socket> socket); //wala na dapat to
  int IsStationary(); //ok
  void setStationary(int value); //ok
  void SetupBuffer(uint32_t numOfEntries, uint32_t entrySize);

protected:
  virtual void StartApplication (void); //ok
  virtual void StopApplication (void); //ok
  
  void SendBundle (uint32_t dstnode, uint32_t packetSize); //ok
  void SendAP (Ipv4Address srcaddr, Ipv4Address dstaddr, uint32_t seqno, Time srctimestamp); //ok
  void PrintBuffers (void); //ok
  void CheckBuffers (uint32_t bundletype); //ok
  int IsDuplicate (Ptr<Packet> pkt, Ptr<Queue> queue); //ok
  int AntipacketExists (Ptr<Packet> pkt); //ok
  void RemoveBundle (Ptr<Packet> pkt); //ok
...
StationaryDtnApp::StationaryDtnApp ()
  : m_socket (0),
    newpkt (0),
    retxpkt (0),
...
void StationaryDtnApp::Setup (Ptr<Node> node){
  m_node = node;
...
void StationaryDtnApp::DstHandleConnectionCreated (Ptr<Socket> s, const Address & addr){
  s->SetRecvCallback (MakeCallback (&StationaryDtnApp::ReceiveBundle, this));
}




void StationaryDtnApp::ReceiveHello (Ptr<Socket> socket){
  Ptr<Packet> packet;
  Address from;
  while (packet = socket->RecvFrom (from)) {
    // std::cout<<"stationary received a hello from someone.\n";
...
void StationaryDtnApp::ScheduleTx (uint32_t dstnode, Time tNext, uint32_t packetsize){

  m_sendEvent = Simulator::Schedule (tNext, &StationaryDtnApp::SendBundle, this, dstnode, packetsize);

}


void StationaryDtnApp::Retransmit (InetSocketAddress sendTo, int32_t id, int32_t retx){
  // std::cout<<"Stationary node retransmitting...\n";
  ...
  while ((i < neighbors) && (neighbor_found == 0)) {
    std::cout<<neighbor_address[i].GetIpv4()<<"\n";
    if (neighbor_address[i].GetIpv4() == sendTo) {
      neighbor_found = 1;
      if ((Simulator::Now ().GetSeconds () - neighbor_last_seen[i]) > 0.1) {
        Simulator::Schedule (Seconds (1.0), &StationaryDtnApp::Retransmit, this, sendTo, id, retx);
        // std::cout<<"Here3\n";
        return;
      }
    } 
...
void StationaryDtnApp::SendMore (InetSocketAddress sendTo, int32_t id, int32_t retx){
  // std::cout<<"Stationary node sending more ...\n";
  int index = 0, found = 0;
  while ((found == 0) && (index < NumFlows)) {
    if ((sendTos[index] == sendTo) && (ids[index] == id) && (retxs[index] == retx) && ((Simulator::Now ().GetSeconds () - firstSendTime[index]) < 750.0)) {
      found = 1;
...
void StationaryDtnApp::PrintBuffers (void){
  ...
  Simulator::Schedule (Seconds (1.0), &StationaryDtnApp::PrintBuffers, this);
}

void StationaryDtnApp::CheckBuffers (uint32_t bundletype){
  Ptr<Packet> packet;
  ...
  packet->RemoveAtEnd (packet->GetSize () - 1472);
      ...
      Simulator::Schedule (Seconds (1.0), &StationaryDtnApp::Retransmit, this, sendTos[NumFlows-1], ids[NumFlows-1], retxs[NumFlows-1]);
    } else {    
  ...
  if (bundletype == 2) {
    if (send_bundle == 0)
      ...
      Simulator::Schedule (Seconds (0.001), &StationaryDtnApp::CheckBuffers, this, 2);
  }
  if (bundletype == 1) {
    if (send_bundle == 0)
      CheckBuffers(0);
  ...
  if (bundletype == 0) {
    if (send_bundle == 0)
      Simulator::Schedule (Seconds (0.01), &StationaryDtnApp::CheckBuffers, this, 2);
    else
      Simulator::Schedule (Seconds (0.001), &StationaryDtnApp::CheckBuffers, this, 2);
  }
}

void StationaryDtnApp::SendBundle (uint32_t dstnode, uint32_t packetsize){
...
void StationaryDtnApp::SendAP (Ipv4Address srcstring, Ipv4Address dststring, uint32_t seqno, Time srctimestamp){
...
int StationaryDtnApp::IsDuplicate (Ptr<Packet> pkt, Ptr<Queue> queue){
...
int StationaryDtnApp::AntipacketExists (Ptr<Packet> pkt){
  Ptr<Packet> cpkt = pkt->Copy();
  int apExists = 0;
  mypacket::TypeHeader tHeader (mypacket::MYTYPE_BNDL);
  mypacket::BndlHeader bndlHeader;
...
void StationaryDtnApp::RemoveBundle (Ptr<Packet> pkt){
  Ptr<Packet> cpkt = pkt->Copy(); 
  mypacket::TypeHeader tHeader (mypacket::MYTYPE_AP);
  mypacket::APHeader apHeader;
  cpkt->RemoveHeader(tHeader);
...
void StationaryDtnApp::ReceiveBundle (Ptr<Socket> socket){
  Address ownaddress;
...
int StationaryDtnApp::IsStationary(){
  return stationary;

}

void StationaryDtnApp::setStationary(int value){  
  stationary = value;

}

void StationaryDtnApp::StartApplication (void){
  m_running = true;
...
void StationaryDtnApp::StopApplication (void){
  m_running = false;
...
void StationaryDtnApp::ConnectionSucceeds (Ptr<Socket> localSocket){
  //std::cout << "TCP connection succeeds at time " << Simulator::Now ().GetSeconds () <<
  //" at node " << m_node->GetId () << "\n";
}

void StationaryDtnApp::ConnectionFails (Ptr<Socket> localSocket)
{
...
void
StationaryDtnApp::SendHello (Ptr<Socket> socket, double endTime, Time pktInterval, uint32_t first)
{
  if (first == 0) {
  ...
  if (success) {
      ...
      socket->Send (pkt);
        Simulator::Schedule (Seconds (0.1), &StationaryDtnApp::SendHello, this, socket, endTime, Seconds (0.1), 0);
    } else
      socket->Close ();
  } else
    Simulator::Schedule (pktInterval, &StationaryDtnApp::SendHello, this, socket, endTime, pktInterval, 0);
}


void StationaryDtnApp::SetupBuffer(uint32_t numOfEntries, uint32_t entrySize){
